...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        checker = scheme_eval(first, env)
        interpreter = rest.map(lambda value: scheme_eval(value, env))
        return scheme_apply(checker, interpreter, env)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        if procedure.expect_env:
            new_list += [env]
        try:
            new_list = procedure.py_func(*new_list)
        ...
        # BEGIN PROBLEM 11
        if not procedure.formals:
            return eval_all(procedure.body, env)
        else:
            return eval_all(procedure.formals, env) + eval_all(procedure.body, env)
        # END PROBLEM 11
    else:
